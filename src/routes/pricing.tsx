    ...
    <>
      ...
      <Section className="bg-secondary/40">
        <div className="mx-auto max-w-3xl">
          ...
          <Accordion type="single" collapsible defaultValue="general-questions-0" className="mt-10">
            {faqCategories.map((category) => {
              const categoryKey = category.title.toLowerCase().replaceAll(" ", "-");

              return (
                <div key={category.title} className="mt-10 first:mt-0">
                  <h3 className="mb-4 text-xl font-semibold">{category.title}</h3>
                  {category.items.map((f, i) => (
                    <AccordionItem key={f.q} value={`${categoryKey}-${i}`} className="rounded-2xl border border-border bg-card px-5 mb-3 data-[state=open]:shadow-sm">
                      <AccordionTrigger className="text-left text-base font-semibold hover:no-underline">{f.q}</AccordionTrigger>
                      <AccordionContent className="font-sans text-sm leading-relaxed text-muted-foreground">{f.a}</AccordionContent>
                    </AccordionItem>
                  ))}
                </div>
              );
